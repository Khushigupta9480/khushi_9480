...
class lmsSignupUser(APIView):
    def post(self,request):
        userdata=lmsSignupSerializer(data=request.data)
        if userdata.is_valid():
            lmsUser.objects.create(**userdata.data) #ORM (Performing insert new user in lmsUser table and passing complete dictionary data i.e. kArgs)
...
class lmsGetUserDetails(APIView):
    def get(self,request):
        result=list(lmsUser.objects.filter().values()) # To fetch a list of all users from lmsUser table
